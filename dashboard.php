...
// $used = intval($used_by_images) + intval($used_by_videos) + intval($used_by_music) + intval($used_by_doc);
$used = $total_size+$document_size+$audio_size+$video_size+$photo_size;
// $collections_id = 1;

// $sql = "SELECT id FROM collections WHERE id = ?";
// $stmt = $conn->prepare($sql);
// $collection_id = isset($_POST['id']) ? $_POST['id'] : NULL;
...
$sql = "SELECT file_name, file_size, is_public, uploaded_date FROM files WHERE user_id = ?";
$stmt = $conn->prepare($sql);
...
// retriving the data from the collection table
// $sql = "SELECT id FROM collections where email=?";
// $stmt = $conn->prepare($sql);
// $stmt->bind_param("s",$email);
// $stmt->execute();
// $result = $stmt->get_result();
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES['file'])) {
  ...
  foreach ($_FILES['file']['name'] as $key => $filename) {
    ...
    $file_size_bytes = $_FILES['file']['size'][$key]/(1024*1024);
    // $file_size_mb = $file_size_bytes / (1024 * 1024);
    $category = ($file_size_bytes < 4) ? 'shorts' : 'full';
    ...
    if (in_array($filetype, $allowTypes)) {
        ...
        } elseif (in_array($filetype, ['pdf', 'doc', 'docx', 'css', 'php', 'java', 'py', 'html', 'sql'])) {
        ...
        // Optional: unique file name to avoid duplicates
        ...
        if (move_uploaded_file($_FILES["file"]["tmp_name"][$key], $targetFilePath)) {
            $stmt = $conn->prepare("INSERT INTO files (file_name, file_size, is_public, user_id, file_path, file_type, category) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("sdiisss", $originalName, $file_size_bytes, $isPublic, $user_id, $targetFilePath, $fileTypeCategory, $category);

            if ($stmt->execute()) {
                echo "<script>alert('File uploaded as $category');</script>";
            } else {
...
// }else{
//   echo "<script>alert('Collection not found.')</script>";
// }
